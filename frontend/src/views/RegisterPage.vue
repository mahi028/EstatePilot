...
<template>
  <div class="flex min-h-[calc(100vh-4rem)] items-center justify-center py-8">
    <div class="w-full max-w-md rounded-xl border border-[var(--color-border-default)] bg-[var(--color-bg-card)] p-8 shadow-sm">
      <h2 class="mb-6 text-center text-2xl font-bold text-[var(--color-text-primary)]">Create an account</h2>
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <!-- Name -->
        <div>
          <label for="name" class="mb-1 block text-sm font-medium text-[var(--color-text-secondary)]">Full name</label>
          <input
            id="name"
            v-model="form.name"
            type="text"
        ...
        <!-- Role -->
        <div>
          <label for="role" class="mb-1 block text-sm font-medium text-[var(--color-text-secondary)]">Role</label>
          <select
            id="role"
            v-model="form.role"
            class="w-full rounded-lg border border-[var(--color-border-default)] bg-[var(--color-bg-input)] px-3 py-2 text-sm text-[var(--color-text-primary)] outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
          ...
          <p v-if="errors.role" class="mt-1 text-xs text-danger-500">{{ errors.role[0] }}</p>
        </div>
        ...
        <!-- Confirm password -->
        <div>
          <label for="confirm_password" class="mb-1 block text-sm font-medium text-[var(--color-text-secondary)]">Confirm password</label>
          <input
            id="confirm_password"
            ...
            class="w-full rounded-lg border border-[var(--color-border-default)] bg-[var(--color-bg-input)] px-3 py-2 text-sm text-[var(--color-text-primary)] outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
          />
          <p v-if="errors.confirm_password" class="mt-1 text-xs text-danger-500">{{ errors.confirm_password[0] }}</p>
        </div>
      ...
      <p class="mt-6 text-center text-sm text-[var(--color-text-muted)]">
        ...
        <RouterLink to="/login" class="font-medium text-primary-600 hover:text-primary-500">Log in</RouterLink>
      </p>
    </div>
  </div>
</template>
